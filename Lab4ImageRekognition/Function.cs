    ...
    public class Function
    {
        ...
        public async Task<S3Event> FunctionHandler(S3Event input, ILambdaContext context)
        {
            foreach(var record in input.Records)
            {
                ...
                {
                    if (label.Confidence >= 90 && tags.Count < 10)
                    {
                ...
                {
                    tagStr = tagStr +"(" + tag.Key + ": " + tag.Value + ") ";
                }
                Console.WriteLine($"Updating detect tags into S3 object {record.S3.Bucket.Name}/{record.S3.Object.Key} metadata: {tagStr}");
                ...
                Console.WriteLine($"Done to update detect tags into S3 object {record.S3.Bucket.Name}/{record.S3.Object.Key} metadata.");

                Console.WriteLine($"Saving the label and image metadata into DynamoDB Table {record.S3.Bucket.Name}/{record.S3.Object.Key} ");
                await CreateTable();
                //Set a local DB context
                DynamoDBContext dynamoContext = new DynamoDBContext(client);
                
                RcImage rcImage = new RcImage
                {
                    ...
                    etag = record.S3.Object.ETag.ToString(),
                    created = DateTime.UtcNow,
                    updated = DateTime.UtcNow
                };
                await dynamoContext.SaveAsync(rcImage, default(System.Threading.CancellationToken));

                Console.WriteLine($"Done to save the label and image metadata into DynamoDB Table {record.S3.Bucket.Name}/{record.S3.Object.Key} ");
